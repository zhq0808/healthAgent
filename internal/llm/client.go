...
// 不关心系统人设、提示词拼装、意图解析或降级策略，那些属于上层（handler）。
package llm
...
// Stream 以流式方式调用大模型：每收到一段增量文本(delta)，就回调 onDelta。
//
// 与 Complete 的区别：Complete 是等全文返回再一把读完(io.ReadAll)；
// Stream 走 SSE，DeepSeek 每生成几个字就下发一帧 `data: {json}`，边到边回调。
//
